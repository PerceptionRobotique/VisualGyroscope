/*!
 \file MPPSSDgyroEstim.cpp
 \brief Mixture of Photometric Potentials (MPP) SSD for spherical camera
 orientation estimation (3 DOFs), exploiting PeR core, core_extended, io,
 features, estimation and sensor_pose_estimation modules
 * example command line :
 *  ./MPPSSDgyroEstim
 /Users/guillaume/Acquisitions/gyrovisu/spherique/gyro/SVMIS/calib/resultats/calib_subdiv3.xml
 3 0.325
 /Users/guillaume/Acquisitions/gyrovisu/spherique/gyro/wheelchairESIGELEC/sequence/subdiv3/
 1 1 850 1
 /Users/guillaume/Acquisitions/gyrovisu/spherique/gyro/wheelchairESIGELEC/sequence/subdiv3/maskFull.png
 1 1 1
 ...
 \param imDir the directory containing the images to process
 ...
 \param Mask the image file of the mask (white pixels are to be considered
 whereas black pixels are not)
 \param nbTries the number of tested initial
 guesses for the optimization (the one leading to the lower MPP-SSD is kept)
 ...
 * considering the Barreto's model from an XML file got from the MV calibration
 * software
 ...
 * angles of the previous image (the resquest image changes only if the MPP-SSD
 * error is greater than a threshold)
 * 4. Save the MMP-SSD at optimal poses, optimal poses, processing times and key
 * images numbers to files
 *
 * \return
...
int main(int argc, char **argv) {
  ...
  if (stereoCam.get_nbsens() >= 1) {
    std::cout << "the stereo rig is made of a "
              << ((prCameraModel *)stereoCam.sen[0])->getName()
              << " camera of intrinsic parameters alpha_u = "
              << ((prCameraModel *)stereoCam.sen[0])->getau()
              << " ; alpha_v = " << ((prCameraModel *)stereoCam.sen[0])->getav()
              << " ; u_0 = " << ((prCameraModel *)stereoCam.sen[0])->getu0()
              << " ; v_0 = " << ((prCameraModel *)stereoCam.sen[0])->getv0();
    if (((prCameraModel *)stereoCam.sen[0])->getType() == Omni)
      std::cout << " ; xi = " << ((prOmni *)stereoCam.sen[0])->getXi();
    std::cout << "..." << std::endl;
  }

  // If a second sensor is loaded, print its parameters and its pose relatively
  // to the first camera
  if (stereoCam.get_nbsens() >= 2) {
    std::cout << "... and a " << ((prCameraModel *)stereoCam.sen[1])->getName()
              << " camera of intrinsic parameters alpha_u = "
              << ((prCameraModel *)stereoCam.sen[1])->getau()
              << " ; alpha_v = " << ((prCameraModel *)stereoCam.sen[1])->getav()
              << " ; u_0 = " << ((prCameraModel *)stereoCam.sen[1])->getu0()
              << " ; v_0 = " << ((prCameraModel *)stereoCam.sen[1])->getv0();
  ...
  // Loading the reference image with respect to which the cost function will be
  // computed
  ...
  for (boost::filesystem::directory_iterator iter(dir), end; iter != end;
       ++iter) {
  ...
  // type d'estimation (0 : gyro pur ; 1 : odometrie ; 2 : odometrie a images
  // cles)
...
//    bool dofs[6] = {false, false, false, true, false, false}; //"compas"
    ...
    // calculer en parallele un fSet_des avec lambda_g /= 10 pour les dernières
    // itérations --> précision accrue, sans perdre de temps
    fSet_des.buildFrom(IS_des, GS, GS_sample, poseJacobianCompute); // Goulot !
    std::cout << "nb features : " << fSet_des.set.size() << std::endl;

    // if there is a file provided as initial poses, they are used instead of
    // other strategies
    if (ficInit) {
      r = v_pv_init[nbPass];
      ...
      if (nbTries > 1) {
        ...
        for (int iTry0 = 0; iTry0 < nbTriesPerDOF[0];
             iTry0++, angle[0] += pasAngulaire) {
          r[3] = angle[0];
          if (dofs[4])
            angle[1] = -pasAngulaire * floor(nbTries * 0.5);
          else
            angle[1] = 0;
          for (int iTry1 = 0; iTry1 < nbTriesPerDOF[1];
               iTry1++, angle[1] += pasAngulaire) {
            r[4] = angle[1];
            if (dofs[5])
              angle[2] = -pasAngulaire * floor(nbTries * 0.5);
            else
              angle[2] = 0;
            for (int iTry2 = 0; iTry2 < nbTriesPerDOF[2];
                 iTry2++, angle[2] += pasAngulaire) {
              r[5] = angle[2];

              dMc.buildFrom(r);
              fSet_req.update(dMc);

              prSSDCmp<prCartesian3DPointVec,
                       prPhotometricGMS<prCartesian3DPointVec>>
                  errorComputer(fSet_req, fSet_des, robust);
              prPhotometricGMS<prCartesian3DPointVec> GS_error =
                  errorComputer.getRobustCost();
    ...
    // register the request feature set over the desired one and save the
    // optimal MPP-SSD
    err.push_back(gyro.track(fSet_des, r, 1.0, robust));

    v_temps.push_back(vpTime::measureTimeMs() - temps);
    std::cout << "Pass " << nbPass << " time : " << v_temps[nbPass] << " ms"
              << std::endl;
    ...
    std::cout << "Pose optim : " << r.t() << " cum : " << r_to_save.t()
              << std::endl;
    ...
    s.str("");
    s.setf(std::ios::right, std::ios::adjustfield);
    s << chemin << "/rotComp/" << std::setfill('0') << std::setw(6) << imNum
      << ".png";
    filename = s.str();
  ...
  // 4. Save the MMP-SSD at optimal poses, optimal poses, processing times and
  // key images numbers to files
