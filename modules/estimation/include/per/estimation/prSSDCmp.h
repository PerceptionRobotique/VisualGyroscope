...
/*!
 \class prSSDCmp<T> prSSDCmp.h <per/prSSDCmp.h>
 \brief Class for comparing sets of features using the Sum of Squared Differences (SSD)
 */
template<typename T>
class prSSDCmp : public prFeaturesComparator<T>
{
public:
    /*!
     ...
     */
    prSSDCmp(prFeaturesSet<T> & fSet1, prFeaturesSet<T> & fSet2, bool _robustEstimation = false) : prFeaturesComparator<T>(), nbFeatures(0), robustEstimation(_robustEstimation)
    {
     ...
     */
    int compareFeatures(prFeaturesSet<T> & fSet1, prFeaturesSet<T> & fSet2)
    {
